...
from applibs.status import (
    ...
    SHORT_URL_GENERATION_SUCCESSFUL,
)
...
class GenerateShortUrlAPI(APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        if not serializer.is_valid():
            ...
            return Response(
                VALID_DATA_NOT_FOUND, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if not url_object:
            ...
            return Response(
                FAILED_TO_GENERATE_SHORT_URL, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        return Response(
            format_output_success(SHORT_URL_GENERATION_SUCCESSFUL, response_data),
